...
let findFrequentSleeper = input => {
  ...
  input
  ->createList
  ->Belt.Array.map(((key, (_total, intervals))) => (key, intervals->createInterval))
  ->Belt.Array.forEach(((key, time)) => time->Belt.Array.forEach(t => {
  ...
  let output = sleepies->Js.Dict.entries->Belt.Array.map(((id, minutes)) =>
    switch minutes->getMostSleepy {
    | Some((day, count)) => (id, day, count)
    | None => (id, "", 0)
    }
  )->Utils.sortInPlaceWith(((_, _, countA), (_, _, countB)) => countB - countA)->Belt.Array.get(0)
