...
let parseDate = row =>
  switch dateRe->Js.String.match_(row) {
  | Some(date) => (date[0], date[1])
  | None => ("", "")
  }

let guardState = guard =>
  switch firstWordRe->Js.String.match_(guard) {
  | Some(w) =>
    switch w[0] {
    | "falls" => FallsAsleep
    | "wakes" => WakeUp
...
let latestGuard = ref("")
let guardId = row =>
  switch guardIdRe->Js.String.match_(row) {
  | Some(id) =>
    latestGuard := id[0]
    id[0] |> Js.String.replace("#", "")
  | None => latestGuard.contents
  }

let sortByDate = (((a, _), _), ((b, _), _)) => Utils.dateAsInt(a) - Utils.dateAsInt(b)
  ...
  input->Belt.Array.map(row => {
    ...
    | WakeUp =>
      ...
      | Some((prevTotal, intervals)) =>
        ...
        let diff = now - fellAsleep.contents
        let newInterval = Utils.range(fellAsleep.contents, now)

        list->Js.Dict.set(id, (diff + prevTotal, Belt.Array.concat([newInterval], intervals)))
      | None => ()
      }
    | _ => ()
    }
...
let createInterval = arr => arr->Belt.Array.map(Belt.List.toArray)->Utils.flatten
