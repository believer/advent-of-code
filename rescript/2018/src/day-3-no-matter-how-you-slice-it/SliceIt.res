...
let parseLine = row =>
  switch Js.String.match_(%re("/\\#(\\d+) @ (\\d+),(\\d+)\\: (\\d+)x(\\d+)/"), row) {
...
let findOverlap = input => createMatrix(input)->Utils.flatten->keep(v => v == "x")->length

let findFabric = input => {
  let matrix = createMatrix(input)

  let output = input->map(row => {
    let (id, x, y, sx, sy) = parseLine(row)
    ...
    block->keep(row => Js.Array.includes("x", row)),
    id,
  ))->keep(((block, _)) => length(block) == 0)->get(0)
